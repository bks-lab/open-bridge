...
"""Shared Bridge CLI context and explicit per-turn completion checks."""
...
def valid_log(path):
    """A deleted, blank or redirected log is not evidence of recorded work."""
    if path.is_symlink() or not path.is_file() or path.parent.is_symlink():
        return False
    with path.open('rb') as stream:
        return any(block.strip() for block in iter(lambda: stream.read(65536), b''))
...
def snapshot(root):
    ...
    return {'head': head, 'files': files, 'log': digest(root / 'work/log.md'), 'work_enabled': work_enabled(root),
            'log_valid': valid_log(root / 'work/log.md')}
...
def needs_log(before, after):
    ...
    return before.get('work_enabled', False) and changed and (
        not after.get('log_valid', False) or before['log'] == after['log'])
...
def doctor(root, client="codex"):
    ...
    for error in errors:
        print('FAIL: ' + error)
    if not errors:
        print(f'PASS: {client.capitalize()} skill entry and available repository knowledge routing')
    print('INFO: start/finish are explicit commands, not automatically installed client hooks.')
    return bool(errors)


def main(client="codex"):
    if client not in ("codex", "vibe"):
        raise ValueError("Unsupported Bridge CLI client")
    ...
    state_dir = root / f'.bridge/{client}-sessions'
    ...
    if needs_log(before, snapshot(root)):
        print('FAIL: work changed since checkpoint without a changed, nonempty regular work/log.md. Add the substantive log row.')
        return 2
    hook = root / 'scripts/worklog-drift-check.sh'
    if before.get('work_enabled'):
        if not hook.is_file() or hook.is_symlink():
            raise RuntimeError('Shared status-drift checker is missing or not a regular file')
